...
@section('title', 'Video Category Language')
...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>Edit Video Category Language Version</span>
            </li>
        </ul>
    </div>
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">Edit Video Category Language Version</h1>
    <!-- END PAGE TITLE-->
    <!-- END PAGE HEADER-->
    <div class="row">
        <div class="col-md-12">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    ...
                    <div class="portlet-body">
                        <form id="form_video_edit" action="/video/category/language/save" method="post" enctype="multipart/form-data">
                            <div class="row">
                                {{ csrf_field() }}
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="control-label">Name</label>
                                        <input type="text" name="name" class="form-control" value="@if (!is_null($videoCategoryLanguage)) {{ $videoCategoryLanguage->name }} @endif" />
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label">Description</label>
                                        <textarea name="description" class="form-control" rows="5">@if (!is_null($videoCategoryLanguage)) {{ $videoCategoryLanguage->description }} @endif</textarea>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="control-label">Name</label>
                                        <div class="form-control">{{ $videoCategory->name }}</div>
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label">Description</label>
                                        <div class="form-control"> {{ $videoCategory->description }} </div>
                                    </div>
                                </div>
                            </div>
                            <div class="form-actions">
                                <div class="margin-top-10">
                                    <input type="hidden" name="video_category_id" id="video_category_id" value="{{ $videoCategory->id }}" />
                                    <input type="hidden" name="language_id" id="language_id" value="{{$language->id }}" />
                                    <input type="hidden" name="id" id="video_category_language_id" value="@if (!is_null($videoCategoryLanguage)) {{ $videoCategoryLanguage->id }} @endif" />
